...
import com.beeveloper.beathub.user.domain.dto.response.UserInstrumentResponseDto;
...
public class UserInstrumentController {
    ...
    public ResponseEntity<UserInstrumentResponseDto> register(
            @RequestHeader(value = "Authorization") String jwtToken,
            @RequestBody UserInstrumentInputDto inputDto) {

        // JWT Token 없으면 401 에러
        if (jwtToken == null) {
            return (ResponseEntity<UserInstrumentResponseDto>) ResponseEntity.status(401);
        }
        ...
        return ResponseEntity.status(200).body(UserInstrumentResponseDto.of(savedUserInstrument));
    }
}
